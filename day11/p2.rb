...
hull[curr_coord] = [1, 0] # [color, num_paintings]
...
while true do
  # run program with current coordinate color as input
  ...
  if outputs.odd?
    ...
    hull[curr_coord] = [1, 0] if hull[curr_coord].nil?

    min_x = curr_coord.x if curr_coord.x < min_x
    max_x = curr_coord.x if curr_coord.x > max_x
    min_y = curr_coord.y if curr_coord.y < min_y
    max_y = curr_coord.y if curr_coord.y > max_y
  end
end

max_y.downto(min_y) do |y|
  min_x.upto(max_x) do |x|
    section = hull[Coord.new(x, y)]
    if section
      print '  ' if section[0].zero?
      print '##' if section[0] == 1
    else
      print '  '
    end
  end
